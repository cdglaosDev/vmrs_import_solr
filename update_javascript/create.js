  ...
  rl.question("Enter your start number: ", async (startNumber) => {
    if (startNumber.trim() && numberPattern.test(startNumber)) {
      rl.question("Enter your last number: ", async (lastNumber) => {
        if (
          lastNumber.trim() &&
          numberPattern.test(lastNumber) &&
          parseInt(startNumber) <= parseInt(lastNumber)
        ) {
          await createVehicle(parseInt(startNumber), parseInt(lastNumber)).then(
            async () => {
          ...
          console.log(
            "Data invalid, please enter only number and start number <= last number"
          );
          startProgram();
        }
      });
...
async function createVehicle(startNumber, lastNumber) {
  ...
  for (let i = startNumber; i <= lastNumber; i++) {
    const vehicle = await vehicleController
      .findVehicleByNumber(i)
      .catch((err) => {
        throw err;
      });
    if (vehicle) {
